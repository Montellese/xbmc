...
void CVideoDatabase::CreateTables()
{
  ...
  m_pDS->exec("CREATE TABLE source ( source_id integer primary key, identifier text NOT NULL, idPath integer NOT NULL, name text NOT NULL, media_types text NOT NULL, settings text)");
...
void CVideoDatabase::CreateViews()
{
  ...
  std::string episodeview = PrepareSQL("CREATE VIEW episode_view AS SELECT "
                                      ...
                                      "  1 - MIN(IFNULL(import_link.import_id, 0), 1) AS local," // 1 if the item is local, otherwise 0
                                      "  MIN(IFNULL(import_link.import_id, 0), 1) AS imported " // 1 if the item is imported, otherwise 0
                                      "FROM episode"
                                      "  JOIN files ON"
  ...
  std::string tvshowview = PrepareSQL("CREATE VIEW tvshow_view AS SELECT "
                                     ...
                                     "  import_link.enabled AS enabled,"
                                     "  importsPath.strPath AS importPath,"
                                     "  MIN(COUNT(DISTINCT(tvshowlinkpath.idPath)) - COUNT(DISTINCT(import_link.import_id)), 1) as local," // 1 if the item is local, otherwise 0
                                     "  MIN(COUNT(DISTINCT(import_link.import_id)), 1) as imported " // 1 if the item is imported, otherwise 0
                                     "FROM tvshow"
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
          ...
          g_passwordManager.IsDatabasePathUnlocked(path, *CMediaSourceSettings::GetInstance().GetSources("video"))))
      {
        ...
        pItem->SetSource(m_pDS->fv(12).get_asString());
        pItem->SetImportPath(m_pDS->fv(14).get_asString());
...
std::vector<CMediaImportSource> CVideoDatabase::GetSources()
{
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      sources.push_back(CMediaImportSource(m_pDS->fv(1).get_asString(), m_pDS->fv(6).get_asString(), m_pDS->fv(3).get_asString(), m_pDS->fv(7).get_asString(), media_types, lastSynced, m_pDS->fv(5).get_asString()));
...
int CVideoDatabase::AddSource(const CMediaImportSource &source)
{
  ...
  try
  {
    ...
    strSQL = PrepareSQL("INSERT INTO source (source_id, identifier, idPath, name, media_types, settings) values(NULL, '%s', %d, '%s', '%s', '%s')",
...
bool CVideoDatabase::SetDetailsForSource(const CMediaImportSource &source)
{
  ...
  try
  {
    ...
    std::vector<MediaType> vecmedia_types(source.GetAvailableMediaTypes().begin(), source.GetAvailableMediaTypes().end());
    strSQL = PrepareSQL("UPDATE source SET idPath=%d, name='%s', media_types='%s', settings='%s' WHERE source_id=%d",
                        idPath, source.GetFriendlyName().c_str(),
                        StringUtils::Join(vecmedia_types, ",").c_str(),
                        source.Settings().ToXml().c_str(),
                        idSource);
    m_pDS->exec(strSQL.c_str());
  ...
  try
  {
    ...
    if (success)
    {
      ...
      strSQL = PrepareSQL("DELETE FROM path WHERE EXISTS "
        ...
        "JOIN import ON sourcePath.idPath = source.idPath "
...
std::vector<CMediaImport> CVideoDatabase::GetImports()
{
  ...
  try
  {
    ...
    strSQL = PrepareSQL("SELECT path.strPath, source.identifier, import.media_type, import.recursive, import.last_sync, import.settings, parentPath.strPath FROM import "
                        "JOIN path ON path.idPath = import.idPath "
                        "JOIN source ON source.source_id = import.source_id "
                        "LEFT JOIN path AS parentPath ON source.idPath = parentPath.idPath");
    m_pDS->query(strSQL.c_str());
    while (!m_pDS->eof())
    {
      ...
      if (m_pDS->fv(3).get_asBool())
        import = CMediaImport::CreateRecursive(m_pDS->fv(0).get_asString(), CMediaTypes::Split(m_pDS->fv(2).get_asString()), CMediaImportSource(m_pDS->fv(1).get_asString()), lastSynced);
      else
        import = CMediaImport::CreateSelective(m_pDS->fv(0).get_asString(), CMediaTypes::Split(m_pDS->fv(2).get_asString()), CMediaImportSource(m_pDS->fv(1).get_asString(), m_pDS->fv(6).get_asString()), lastSynced);
