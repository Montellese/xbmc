...
void CGUIDialogVideoSettings::OnSettingChanged(const CSetting *setting)
{
           ...
           settingId == SETTING_VIDEO_NONLIN_STRETCH)
  {
    ...
    videoSettings.m_Brightness = static_cast<const CSettingInt*>(setting)->GetValue();
  else if (settingId == SETTING_VIDEO_CONTRAST)
    videoSettings.m_Contrast = static_cast<const CSettingInt*>(setting)->GetValue();
  else if (settingId == SETTING_VIDEO_GAMMA)
    videoSettings.m_Gamma = static_cast<const CSettingInt*>(setting)->GetValue();
...
//  entries.push_back(make_pair(?????, VS_SCALINGMETHOD_NEDI));
  ...
  if (g_renderManager.Supports(RENDERFEATURE_BRIGHTNESS))
    AddPercentageSlider(groupVideoPlayback, SETTING_VIDEO_BRIGHTNESS, 464, 0, videoSettings.m_Brightness, 14047, 1, 464, usePopup);
  if (g_renderManager.Supports(RENDERFEATURE_CONTRAST))
    AddPercentageSlider(groupVideoPlayback, SETTING_VIDEO_CONTRAST, 465, 0, videoSettings.m_Contrast, 14047, 1, 464, usePopup);
  if (g_renderManager.Supports(RENDERFEATURE_GAMMA))
    AddPercentageSlider(groupVideoPlayback, SETTING_VIDEO_GAMMA, 466, 0, videoSettings.m_Gamma, 14047, 1, 464, usePopup);
