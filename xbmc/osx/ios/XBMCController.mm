  ...
  { 
    ...
    if( [sender state] == UIGestureRecognizerStateChanged )
    {
      ...
      if( bNotify )
      {
        ...
        {
          CGenericTouchActionHandler::Get().OnTouchGesturePanStart((float)point.x, (float)point.y);
    ...
    if( touchBeginSignaled && [sender state] == UIGestureRecognizerStateEnded )
    {
      ...
      CGenericTouchActionHandler::Get().OnTouchGesturePanEnd((float)lastGesturePoint.x, (float)lastGesturePoint.y,
                                                             (float)0.0, (float)0.0, 
                                                             (float)velocity.x, (float)velocity.y);
