...
bool CMediaImportSourceEvent::Execute() const
{
  ...
  params.push_back(
      StringUtils::Format("import://{}/", CURL::Encode(m_source.GetIdentifier())));
...
CMediaImportEvent::CMediaImportEvent(const CMediaImport& import,
                                     ...
                                     bool removed,
                                     EventLevel level /* = EventLevel::Information */)
  : CUniqueEvent(fmt::format(g_localizeStrings.Get(39581),
                             "provider"_a = import.GetSource().GetFriendlyName(),
                             "mediatypes"_a = CMediaTypes::ToLabel(import.GetMediaTypes())),
                 description,
