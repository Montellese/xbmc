...
std::shared_ptr<ADDON::CMediaImporter> GetAddon(const std::string& addonId)
{
  ...
  if (!CServiceBroker::GetAddonMgr().GetAddon(addonId, addon, ADDON::ADDON_MEDIAIMPORTER,
...
void CAddonMediaImporter::SetProviderSettingsLoaded(HandleType handle, bool settingsLoaded) throw(
    InvalidAddonMediaImporterHandleException)
{
  SetSuccess(handle, settingsLoaded, CAddonMediaImporterExecutor::Action::LoadSourceSettings);
}

void CAddonMediaImporter::SetImportSettingsLoaded(HandleType handle, bool settingsLoaded) throw(
    InvalidAddonMediaImporterHandleException)
{
