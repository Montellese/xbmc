...
void CMediaImportManager::Initialize(IMediaImportRepositoryManager* repositoryManager /* = nullptr */)
{
  if (m_initialized)
    return;
  ...
  // if we have an import repository manager get and register all its repositories
  if (repositoryManager != nullptr)
  {
    const auto repositories = repositoryManager->GetImportRepositories();
    for (const auto& repository : repositories)
      RegisterImportRepository(repository);
  }
}
  ...
  std::vector<CMediaImport> addedImports;
  {
    ...
    for (const auto& itMediaTypes : mediaTypes)
    {
      ...
      for (const auto& mediaType : itMediaTypes)
      {
        ...
        if (m_importHandlersMap.find(mediaType) == m_importHandlersMap.end())
        {
          m_logger->error("unable to add new import from source \"{}\" "
