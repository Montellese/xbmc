...
CMediaImportManager& CMediaImportManager::GetInstance()
{
...
std::vector<MediaImportHandlerConstPtr> CMediaImportManager::GetImportHandlers(const MediaTypes& mediaTypes) const
{
  if (mediaTypes.empty())
...
GroupedMediaTypes CMediaImportManager::GetSupportedMediaTypesOrdered(bool reversed /* = false */) const
{
...
GroupedMediaTypes CMediaImportManager::GetSupportedMediaTypesOrdered(const MediaTypes& mediaTypes, bool reversed /* = false */) const
{
  GroupedMediaTypes mediaTypesOrdered;
  if (mediaTypes.empty())
...
std::vector<GroupedMediaTypes> CMediaImportManager::GetSupportedMediaTypesGrouped(const MediaTypes &mediaTypes) const
{
...
{
  GroupedMediaTypes groupedMediaTypes;
  if (mediaType.empty())
...
bool CMediaImportManager::AddSource(const std::string& sourceID, const std::string& basePath, const std::string& friendlyName, const std::string& iconUrl /* = "" */, const MediaTypes& mediaTypes /* = MediaTypes() */)
{
  return AddSourceInternal(sourceID, basePath, friendlyName, iconUrl, mediaTypes, false);
}

bool CMediaImportManager::AddSourceSync(const std::string& sourceID, const std::string& basePath, const std::string& friendlyName, const std::string& iconUrl /* = "" */, const MediaTypes& mediaTypes /* = MediaTypes() */)
{
  return AddSourceInternal(sourceID, basePath, friendlyName, iconUrl, mediaTypes, true);
}

bool CMediaImportManager::ActivateSource(const std::string& sourceID, const std::string& basePath /* = "" */, const std::string& friendlyName /* = "" */, const std::string& iconUrl /* = "" */)
{
  if (sourceID.empty())
  {
    CLog::Log(LOGWARNING, "CMediaImportManager: unable to register source with invalid identifier \"%s\"", sourceID.c_str());
    return false;
  }

  CMediaImportSource currentSource(sourceID);

  CSingleLock sourcesLock(m_sourcesLock);
  auto&& itSource = m_sources.find(sourceID);
  if (itSource == m_sources.end() || !FindSource(sourceID, currentSource))
    return false;

  // update any possibly changed values
  CMediaImportSource updatedSource = currentSource;
  // update any provided values from the existing source
  if (!basePath.empty())
    updatedSource.SetBasePath(basePath);
  ...
  for (auto& repository : m_importRepositories)
  {
    ...
    if (!repository->GetSource(updatedSource.GetIdentifier(), currentSource))
    {
  ...
  if (updated)
  {
    CLog::Log(LOGDEBUG, "CMediaImportManager: source %s (%s) updated",
      updatedSource.GetFriendlyName().c_str(), sourceID.c_str());
...
void CMediaImportManager::DeactivateSource(const std::string& sourceID)
{
  ...
  CLog::Log(LOGDEBUG, "CMediaImportManager: source %s deactivated", sourceID.c_str());
  sourcesLock.Leave();
...
bool CMediaImportManager::UpdateSource(const CMediaImportSource &source)
{
  ...
  for (auto& repository : m_importRepositories)
  {
    if (repository->UpdateSource(source))
      updated = true;
  }
  repositoriesLock.Leave();
  sourcesLock.Leave();
...
bool CMediaImportManager::AddSelectiveImport(const std::string &sourceID, const std::string &path, const GroupedMediaTypes &mediaTypes)
{
  if (sourceID.empty() || path.empty() || mediaTypes.empty())
  {
    CLog::Log(LOGERROR, "CMediaImportManager: unable to add new selective import from source \"%s\" with path \"%s\" and media type \"%s\"",
      sourceID.c_str(), path.c_str(), CMediaTypes::Join(mediaTypes).c_str());
    return false;
  }
...
bool CMediaImportManager::AddRecursiveImport(const std::string &sourceID, const std::string &path, const GroupedMediaTypes &mediaTypes)
{
  if (sourceID.empty() || path.empty() || mediaTypes.empty())
  {
    CLog::Log(LOGERROR, "CMediaImportManager: unable to add new recursive import from source \"%s\" with path \"%s\" and media type \"%s\"",
              sourceID.c_str(), path.c_str(), CMediaTypes::Join(mediaTypes).c_str());
    return false;
  }
...
bool CMediaImportManager::AddRecursiveImports(const std::string &sourceID, const std::string &path, const std::set<GroupedMediaTypes> &mediaTypes)
{
  if (sourceID.empty() || path.empty() || mediaTypes.empty())
  {
    CLog::Log(LOGERROR, "CMediaImportManager: unable to add new recursive imports from source \"%s\" with path \"%s\" and media types (%zu)",
  ...
  {
    CLog::Log(LOGWARNING, "CMediaImportManager: trying to add new recursive imports from unknown source \"%s\"", sourceID.c_str());
    return false;
  }
  ...
  for (const auto& itMediaTypes : mediaTypes)
  {
    ...
    {
      CLog::Log(LOGERROR, "CMediaImportManager: unable to add already existing recursive import from source \"%s\" with path \"%s\" and media type \"%s\"",
                sourceID.c_str(), path.c_str(), CMediaTypes::Join(itMediaTypes).c_str());
      continue;
    }
    ...
    for (const auto& mediaType : itMediaTypes)
    {
      ...
      {
        CLog::Log(LOGERROR, "CMediaImportManager: unable to add new recursive import from source \"%s\" with path \"%s\" and media type \"%s\" because there is no matching import handler available",
    ...
    {
      CLog::Log(LOGERROR, "CMediaImportManager: failed to add new recursive import for source \"%s\" with path \"%s\" and media type \"%s\" to any import repository",
                sourceID.c_str(), path.c_str(), CMediaTypes::Join(itMediaTypes).c_str());
      continue;
    }
...
void CMediaImportManager::RemoveImport(const std::string& path, const GroupedMediaTypes &mediaTypes)
{
  ...
  CLog::Log(LOGINFO, "CMediaImportManager: import removal task for %s items of %s started", CMediaTypes::Join(import.GetMediaTypes()).c_str(), import.GetPath().c_str());
}
...
std::vector<CMediaImport> CMediaImportManager::GetImportsByMediaType(const GroupedMediaTypes &mediaTypes) const
{
  std::vector<CMediaImport> imports;
  if (mediaTypes.empty())
    return imports;
...
bool CMediaImportManager::Import(const std::string& path, const GroupedMediaTypes& mediaTypes)
{
  if (path.empty() || mediaTypes.empty())
  {
    CLog::Log(LOGWARNING, "CMediaImportManager: unable to import from invalid path \"%s\" or without media types (%s)",
              path.c_str(), CMediaTypes::Join(mediaTypes).c_str());
    return false;
  }
...
bool CMediaImportManager::AddSourceInternal(const std::string& sourceID, const std::string& basePath, const std::string& friendlyName, const std::string& iconUrl, const MediaTypes& mediaTypes, bool synchronous)
{
  if (sourceID.empty() || friendlyName.empty())
  {
    CLog::Log(LOGWARNING, "CMediaImportManager: unable to add source with invalid identifier \"%s\" or friendly name \"%s\"",
  ...
  if (synchronous)
    return OnTaskComplete(processorJob->DoWork(), processorJob->GetCurrentTask());
...
bool CMediaImportManager::AddImport(const CMediaImport &import)
{
  ...
  CEventLog::GetInstance().Add(EventPtr(new CMediaImportEvent(import, 39067)));
  return true;
}

bool CMediaImportManager::FindImport(const std::string &path, const GroupedMediaTypes &mediaTypes, CMediaImport &import) const
{
  CSingleLock repositoriesLock(m_importRepositoriesLock);
  // try to find the import in at least one of the repositories
  return std::any_of(m_importRepositories.begin(), m_importRepositories.end(),
    [&path, &mediaTypes, &import](const MediaImportRepositoryPtr& repository) { return repository->GetImport(path, mediaTypes, import); });
}
...
bool CMediaImportManager::UpdateImportedItem(const CFileItem &item)
{
  ...
  if (mediaType.empty())
  {
    CLog::Log(LOGWARNING, "CMediaImportManager: unable to update %s due to unknown media type",
      item.GetPath().c_str());
    return false;
  }

  // get all media types belonging to the item's media type
  GroupedMediaTypes mediaTypes = GetGroupedMediaTypes(mediaType);
  if (mediaTypes.empty())
  {
    CLog::Log(LOGWARNING, "CMediaImportManager: unable to update %s due to unsupported media type (%s)",
      item.GetPath().c_str(), mediaType.c_str());
    return false;
  }
  ...
  {
    CLog::Log(LOGWARNING, "CMediaImportManager: unable to update %s due to no import found for %s of %s",
      item.GetPath().c_str(), importPath.c_str(), CMediaTypes::Join(mediaTypes).c_str());
    return false;
  }
  ...
  {
    CLog::Log(LOGWARNING, "CMediaImportManager: unable to update item %s on inactive source %s",
      item.GetPath().c_str(), import.GetSource().GetIdentifier().c_str());
    return false;
  }
  ...
  CLog::Log(LOGINFO, "CMediaImportManager: import update task for %s on %s started",
    item.GetPath().c_str(), import.GetPath().c_str());
    ...
    if (!sourceID.empty())
    {
      ...
      if (itSource != m_sources.end() && itSource->second.removing)
      {
        ...
        if (FindSource(sourceID, source))
        {
          ...
          CEventLog::GetInstance().Add(EventPtr(new CMediaImportSourceEvent(source, 39051, true)));
        }
      }
    }
  }
...
bool CMediaImportManager::OnTaskComplete(bool success, const IMediaImportTask *task)
{
  ...
  if (taskType == MediaImportTaskType::SourceRegistration)
  {
    ...
    CEventLog::GetInstance().AddWithNotification(EventPtr(new CMediaImportSourceEvent(source, 39050)));
  }
  else if (taskType == MediaImportTaskType::Synchronisation)
  {
    ...
    if (!success)
    {
      CEventLog::GetInstance().Add(EventPtr(new CMediaImportEvent(import, 39053, EventLevel::Error)));
      return false;
    }
    ...
    CEventLog::GetInstance().Add(EventPtr(new CMediaImportEvent(import, 39054)));
  }
  else if (taskType == MediaImportTaskType::Removal)
  {
    ...
    if (!success)
    {
      CEventLog::GetInstance().Add(EventPtr(new CMediaImportEvent(import, 39078, true, EventLevel::Error)));
      return false;
    }
    ...
    CEventLog::GetInstance().Add(EventPtr(new CMediaImportEvent(import, 39079, true)));
  }
