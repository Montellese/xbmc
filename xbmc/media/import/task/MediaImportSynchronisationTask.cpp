...
bool CMediaImportSynchronisationTask::DoWork()
{
  ...
  GetProgressBarHandle(StringUtils::Format(g_localizeStrings.Get(39061).c_str(),
  ...
  for (const auto& item : m_items)
  {
    ...
    switch (item.first)
    {
      case MediaImportChangesetTypeAdded:
        SetProgressText(StringUtils::Format(g_localizeStrings.Get(39062).c_str(), itemLabel.c_str()));
        m_importHandler->AddImportedItem(m_import, item.second.get());
        break;

      case MediaImportChangesetTypeChanged:
        SetProgressText(StringUtils::Format(g_localizeStrings.Get(39063).c_str(), itemLabel.c_str()));
        m_importHandler->UpdateImportedItem(m_import, item.second.get());
        break;

      case MediaImportChangesetTypeRemoved:
        SetProgressText(StringUtils::Format(g_localizeStrings.Get(39064).c_str(), itemLabel.c_str()));
