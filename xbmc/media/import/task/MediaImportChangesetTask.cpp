...
CMediaImportChangesetTask::CMediaImportChangesetTask(const CMediaImport &import, MediaImportHandlerPtr importHandler, const std::vector<CFileItemPtr> &localItems, const ChangesetItems &retrievedItems)
...
bool CMediaImportChangesetTask::DoWork()
{
  ...
  GetProgressBarHandle(StringUtils::Format(g_localizeStrings.Get(39059).c_str(), m_import.GetSource().GetFriendlyName().c_str()));
  SetProgressText(StringUtils::Format(g_localizeStrings.Get(39060).c_str(), CMediaTypes::GetPluralLocalization(m_importHandler->GetMediaType()).c_str()));
