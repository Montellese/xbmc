...
MediaImportChangesetType CVideoImportHandler::DetermineChangeset(const CMediaImport &import, CFileItem* item, std::vector<CFileItemPtr>& localItems)
{
...
CFileItemPtr CVideoImportHandler::FindMatchingLocalItem(const CFileItem* item, std::vector<CFileItemPtr>& localItems) const
{
...
MediaImportChangesetType CVideoImportHandler::DetermineChangeset(const CMediaImport &import, CFileItem* item, CFileItemPtr localItem, std::vector<CFileItemPtr>& localItems, bool updatePlaybackMetadata)
{
  ...
  localItems.erase(std::remove(localItems.begin(), localItems.end(), localItem), localItems.end());
...
void CVideoImportHandler::PrepareItem(const CMediaImport &import, CFileItem* pItem)
{
  ...
  int idPath = m_db.AddPath(import.GetPath(), sourcePath);
  ...
  pItem->GetVideoInfoTag()->m_basePath = import.GetSource().GetBasePath();
