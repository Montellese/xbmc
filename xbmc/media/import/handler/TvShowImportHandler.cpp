...
{
  return left.m_strTitle == right.m_strTitle
  ...
  if (!tvshows.IsEmpty())
  {
    ...
    for (int i = 0; i < tvshows.Size();)
    {
      ...
      if ((tvshowInfo->HasUniqueID() && tvshowInfo->GetUniqueID() == info->GetUniqueID()) ||
          (tvshowInfo->HasYear() && tvshowInfo->GetYear() == info->GetYear() && tvshowInfo->m_strTitle == info->m_strTitle))
      {
        exists = true;
        break;
      }
...
{
  CFileItemList tvshows;
  if (!videodb.GetTvShowsByWhere("videodb://tvshows/titles/?imported&import=" + CURL::Encode(import.GetPath()),
    CDatabase::Filter(), tvshows, SortDescription(),
    import.GetSettings().UpdateImportedMediaItems() ? VideoDbDetailsAll : VideoDbDetailsNone))
    return false;

  for (int i = 0; i < tvshows.Size(); ++i)
    items.push_back(tvshows.Get(i));

  return true;
}

CFileItemPtr CTvShowImportHandler::FindMatchingLocalItem(const CFileItem* item, std::vector<CFileItemPtr>& localItems) const
{
  const auto& localItem = std::find_if(localItems.cbegin(), localItems.cend(),
    [&item](const CFileItemPtr& localItem) { return IsSameTVShow(*item->GetVideoInfoTag(), *localItem->GetVideoInfoTag()); });
...
MediaImportChangesetType CTvShowImportHandler::DetermineChangeset(const CMediaImport &import, CFileItem* item, CFileItemPtr localItem, std::vector<CFileItemPtr>& localItems, bool updatePlaybackMetadata)
{
