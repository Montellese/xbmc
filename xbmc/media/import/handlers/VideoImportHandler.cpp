    ...
    const std::vector<CFileItemPtr>& localItems) const
{
  ...
  // determine the path of the imported item
  auto itemPath = item->GetVideoInfoTag()->GetPath();
  if (itemPath.empty())
    itemPath = item->GetPath();

  const auto& localItem = std::find_if(
      localItems.cbegin(), localItems.cend(), [&itemPath](const CFileItemPtr localItem) {
        return localItem->HasVideoInfoTag() && localItem->GetVideoInfoTag()->GetPath() == itemPath;
      });
...
bool CVideoImportHandler::RemoveImportedItems(const CMediaImport& import)
{
  ...
  bool success = RemoveImportedItems(m_db, import, false);
...
void CVideoImportHandler::PrepareItem(CVideoDatabase& videodb,
                                      ...
                                      CFileItem* item)
{
  if (item == nullptr || !item->HasVideoInfoTag() || !import.IsValid())
    return;
  ...
  item->SetSource(import.GetSource().GetIdentifier());

  auto videoInfoTag = item->GetVideoInfoTag();

  // make sure the video info tag has a proper path
  if (videoInfoTag->GetPath().empty())
  ...
  if (!item->m_bIsFolder)
  {
    // explicitly add the file with the source identifier as the parent path
    videoInfoTag->SetFileNameAndPath(item->GetPath());
    videoInfoTag->m_iFileId = videodb.AddFile(*videoInfoTag, sourceId);
  }
}
                                            ...
                                            bool reset)
{
  ...
  if (reset)
    videodb.SetPlayCount(*item, videoInfoTag->GetPlayCount(), videoInfoTag->m_lastPlayed, false);

  // clean resume bookmark
  if (reset)
    videodb.DeleteResumeBookMark(*item, false);

  if (videoInfoTag->GetResumePoint().IsPartWay())
    videodb.AddBookMarkToFile(item->GetPath(), videoInfoTag->GetResumePoint(), CBookmark::RESUME);
}
...
bool CVideoImportHandler::SetImportForItem(CVideoDatabase& videodb,
                                           ...
                                           int idFilesystem /* = -1 */)
{
  return videodb.SetImportForItem(item->GetVideoInfoTag()->m_iDbId, GetMediaType(), import,
                                  idFilesystem);
}
...
void CVideoImportHandler::RemoveFile(CVideoDatabase& videodb, const CFileItem* item)
{
