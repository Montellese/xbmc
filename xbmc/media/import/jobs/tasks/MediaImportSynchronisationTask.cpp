  ...
  for (const auto& item : m_items)
  {
    ...
    switch (item.first)
    {
      case MediaImportChangesetType::Added:
        SetProgressText(
            StringUtils::Format(g_localizeStrings.Get(39562), "mediaitem"_a = itemLabel));
        m_importHandler->AddImportedItem(m_import, item.second.get());
        break;

      case MediaImportChangesetType::Changed:
        SetProgressText(
            StringUtils::Format(g_localizeStrings.Get(39563), "mediaitem"_a = itemLabel));
        m_importHandler->UpdateImportedItem(m_import, item.second.get());
        break;

      case MediaImportChangesetType::Removed:
        SetProgressText(
            StringUtils::Format(g_localizeStrings.Get(39564), "mediaitem"_a = itemLabel));
