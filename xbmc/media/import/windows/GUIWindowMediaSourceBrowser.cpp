    ...
    case GUI_MSG_CLICKED:
    {
      ...
      if (m_viewControl.HasControl(iControl)) // list/thumb control
      {
        ...
        if (actionId == ACTION_SHOW_INFO || actionId == ACTION_DELETE_ITEM)
        {
          ...
          if (item->GetProperty(PROPERTY_IMPORT_MEDIATYPES).empty())
          {
...
bool CGUIWindowMediaSourceBrowser::OnSourceSynchronise(const CFileItemPtr& item)
{
  ...
  KODI::MESSAGING::HELPERS::ShowOKDialogText(
      39610, StringUtils::Format(g_localizeStrings.Get(39611), "provider"_a = item->GetLabel()));
  return false;
}
...
bool CGUIWindowMediaSourceBrowser::OnSourceInfo(const CFileItemPtr& item)
{
  ...
  {
    KODI::MESSAGING::HELPERS::ShowOKDialogText(
        39700, StringUtils::Format(g_localizeStrings.Get(39614), "provider"_a = item->GetLabel()));
    return false;
  }
...
bool CGUIWindowMediaSourceBrowser::OnImportSynchronise(const CFileItemPtr& item)
{
  ...
  KODI::MESSAGING::HELPERS::ShowOKDialogText(
      39612, StringUtils::Format(g_localizeStrings.Get(39613), "import"_a = item->GetLabel()));
  return false;
}
                                                ...
                                                bool allowSync /* = true */)
{
  ...
  {
    KODI::MESSAGING::HELPERS::ShowOKDialogText(
        39701, StringUtils::Format(g_localizeStrings.Get(39615), "import"_a = item->GetLabel()));
    return false;
  }
...
bool CGUIWindowMediaSourceBrowser::OnImportDelete(const CFileItemPtr& item)
{
  ...
  CServiceBroker::GetMediaImportManager().RemoveImport(sourceId, mediaTypes);
  return true;
}
...
void CGUIWindowMediaSourceBrowser::OnImportAdded(const std::string& sourceId,
...
void CGUIWindowMediaSourceBrowser::OnImportUpdated(const std::string& sourceId,
                                                   const GroupedMediaTypes& mediaTypes)
{
  if (m_vecItems->GetContent() == "imports")
  {
    // only refresh the list if the updated import is part of it
    CFileItemPtr item = GetImportItem(sourceId, mediaTypes);
    if (item != nullptr)
      RefreshList(true);
  }
}

void CGUIWindowMediaSourceBrowser::OnImportRemoved(const std::string& sourceId,
                                                   const GroupedMediaTypes& mediaTypes)
{
  if (m_vecItems->GetContent() == "imports")
  {
...
CFileItemPtr CGUIWindowMediaSourceBrowser::GetImportItem(const std::string& sourceId,
...
void CGUIWindowMediaSourceBrowser::GetContextButtons(int itemNumber, CContextButtons& buttons)
{
  ...
  bool isSource = !sourceId.empty() && !item->HasProperty(PROPERTY_IMPORT_MEDIATYPES);
  bool isImport = !isSource;
...
bool CGUIWindowMediaSourceBrowser::OnClick(int iItem, const std::string& player /* = "" */)
{
  ...
  if (!item->m_bIsFolder)
  {
    ...
    if (!sourceId.empty() && item->HasProperty(PROPERTY_IMPORT_MEDIATYPES))
      return OnImportInfo(item);
  }
...
bool CGUIWindowMediaSourceBrowser::AddNewImport(CFileItemPtr newImportItem, const CURL& url)
{
  ...
  if (!mediaImportManager.AddImport(sourceId, mediaTypesToImport))
  {
...
bool CGUIWindowMediaSourceBrowser::AddNewSource(CFileItemPtr newImportItem, const CURL& url)
{
  ...
  {
    m_logger->info("source {} already exists", discoveredSource);
    KODI::MESSAGING::HELPERS::ShowOKDialogText(
        CVariant{(*selectedImporter)->GetSourceLookupProtocol()},
        CVariant{StringUtils::Format(g_localizeStrings.Get(39619),
                                     "provider"_a = discoveredSource.GetFriendlyName())});
    return true;
  }
  ...
  if (!result)
  {
    m_logger->error("failed to add discovered source {}", discoveredSource);
    KODI::MESSAGING::HELPERS::ShowOKDialogText(
        CVariant{(*selectedImporter)->GetSourceLookupProtocol()},
        CVariant{g_localizeStrings.Get(39618)});
    return false;
