...
public:
  ...
  */
   ...
   */
  const GroupedMediaTypes& GetMediaTypes() const { return m_import.GetMediaTypes(); }
  ...
  /*
   * \brief Prepares and loads the provided source's settings based on the importers functionality.
   *
   * \param source Source to be prepared and loaded
   * \return True if the source's settings have been successfully prepared and loaded, false otherwise
   */
  virtual bool LoadSourceSettings(CMediaImportSource& source) const { return true; }
  /*
  * \brief Saves and unloads the provided source's settings based on the importers functionality.
  *
  * \param source Source to be unloaded
   * \return True if the source's settings have been successfully saved and unloaded, false otherwise
  */
  virtual bool UnloadSourceSettings(CMediaImportSource& source) const { return true; }
...
typedef std::shared_ptr<IMediaImporter> MediaImporterPtr;
typedef std::shared_ptr<const IMediaImporter> MediaImporterConstPtr;
