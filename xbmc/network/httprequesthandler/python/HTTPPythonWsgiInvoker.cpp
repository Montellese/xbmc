...
void CHTTPPythonWsgiInvoker::executeScript(void *fp, const std::string &script, void *module, void *moduleDict)
{
  ...
  try
  {
    ...
    cgiEnvironment = createCgiEnvironment(m_request);
...
std::map<std::string, std::string> CHTTPPythonWsgiInvoker::createCgiEnvironment(const HTTPPythonRequest* httpRequest)
{
  ...
  // SCRIPT_NAME
  environment.insert(std::make_pair("SCRIPT_NAME", httpRequest->path));

  // TODO: PATH_INFO
  environment.insert(std::make_pair("PATH_INFO", ""));
