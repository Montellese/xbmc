...
static bool Search(CMediaImportRetrievalTask *task, PLT_DeviceDataReference &device, const MediaType &mediaType, std::vector<CFileItemPtr> &items)
{
  ...
  task->GetProgressBarHandle()->SetText(StringUtils::Format(g_localizeStrings.Get(39018).c_str(), CMediaTypes::GetPluralLocalization(supportedMediaType->mediaType).c_str()));
...
bool CUPnPMediaImporter::CanImport(const std::string& path) const
{
  ...
  if (!CSettings::GetInstance().GetBool("services.upnpimportcompatibleonly"))
    return true;
...
bool CUPnPMediaImporter::Import(CMediaImportRetrievalTask* task) const
{
  ...
  const GroupedMediaTypes &importedMediaTypes = GetImport().GetMediaTypes();
  for (const auto& importedMediaType : importedMediaTypes)
  {
    bool success = false;
    for (size_t i = 0; i < SupportedMediaTypesSize; i++)
    {
