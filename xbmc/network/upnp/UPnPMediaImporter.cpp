...
static bool Search(CMediaImportRetrievalTask *task, PLT_DeviceDataReference &device, const MediaType &mediaType, std::vector<CFileItemPtr> &items)
{
  ...
  task->SetProgressText(StringUtils::Format(g_localizeStrings.Get(39068).c_str(), CMediaTypes::GetPluralLocalization(supportedMediaType->mediaType).c_str()));
...
bool CUPnPMediaImporter::Import(CMediaImportRetrievalTask* task) const
{
  ...
  for (const auto& importedMediaType : GetImport().GetMediaTypes())
  {
    bool success = false;
    for (size_t i = 0; i < SupportedMediaTypesSize; i++)
    {
    ...
    for (auto& item : items)
    {
      if (URIUtils::PathHasParent(item->GetPath(), GetPath()))
