    ...
    void ListItem::setInfo(const char* type, const InfoLabelDict& infoLabels)
    {
      ...
      else if (StringUtils::CompareNoCase(type, "pictures") == 0)
      {
        ...
        if (hasDeprecatedInfoLabel)
        {
          CLog::Log(
            LOGWARNING,
            "Setting most picture properties through ListItem.setInfo() is deprecated and might be "
            "removed in future Kodi versions. Please use the respective setter in InfoTagPicture.");
        }
      }
