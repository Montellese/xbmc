...
void InfoTagPicture::setInfo(const CVariant& info)
{
  if (info.empty())
    return;
...
void InfoTagPicture::setInfoRaw(CPictureInfoTag* infoTag, const CVariant& info)
{
  for (const auto& prop : info.asMap())
  {
    auto key = StringUtils::ToLower(prop.first);
    if (StringUtils::StartsWith(key, "exif:"))
      key = StringUtils::Mid(key, 5);
    ...
    if (key == "resolution")
    {
      int width = -1;
      int height = -1;
      DictUtils::ProcessObject(prop.first, value, {
        std::make_pair("width", DictUtils::ObjectProperty{[&width](const std::string& key, const CVariant& value) { width = DictUtils::CheckAndGetIntegerProperty(key, value); }, true }),
        std::make_pair("height", DictUtils::ObjectProperty{[&height](const std::string& key, const CVariant& value) { height = DictUtils::CheckAndGetIntegerProperty(key, value); }, true }),
        });

      setResolutionRaw(infoTag, width, height);
    }
    else if (key == "datetimetaken" || key == "exiftime")
      setDateTimeTakenRaw(infoTag, DictUtils::CheckAndGetStringProperty(prop.first, value));
    else
      CLog::Log(LOGWARNING, "InfoTagPicture.setInfo: unknown property \"{}\"", key);
  }
}
